...
Public Class AccountController
	...
	' ###########################################################################################
	' ###########################################################################################
	'Chee - My Own Functions!

	' GET api/CurrentUserInfo
	<Route("CurrentUserInfo")>
	Public Async Function CurrentUserInfo() As Task (Of cDataHelper)
		Dim sAspNetUsersID As String = User.Identity.GetUserId()

		Dim oDataHelper As New cDataHelper

		Dim oApplicationUser As ApplicationUser = Await UserManager.FindByIdAsync(sAspNetUsersID)
		If oApplicationUser Is Nothing Then
			oDataHelper.Result = False
			Return oDataHelper
		End If

		Dim oBusinessRule_Student As New cBusinessRule_Student
		Dim oObject_Out As cStudent = oBusinessRule_Student.GetStudentAndHobby_ViaAspNetUsersID(sAspNetUsersID)
		oBusinessRule_Student = Nothing
		
		oDataHelper.AspNetUsersID = User.Identity.GetUserId
		oDataHelper.FullName = oApplicationUser.FullName

		If oObject_Out Is Nothing Then
			oDataHelper.Result = False
			Return oDataHelper
		End If
